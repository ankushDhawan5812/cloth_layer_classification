...
class camPublisher(Node):
    ...
    def flag_callback(self, msg):
        if msg.data:
            ...
            csv_filename = f'/home/armlab/Documents/soft_manipulation/wrench_data/{self.num_layers}_layer/wrench_data_{self.trial_num}.csv'
            self.read_rosbag_to_csv(bag_name, '/RunCamera/force', csv_filename)
    ...
    # def read_rosbag_to_csv(self, rosbag_file, topic_name, csv_file):
    #     self.get_logger().info(f"WRITING TO CSV")
    ...
    #             # Iterate over messages in the ROS bag
    #             for connection, timestamp, rawdata in reader.messages('/RunCamera/force'):
    #                 # Deserialize the raw data into a WrenchStamped message
    #                 msg = deserialize_cdr(rawdata, connection.msgtype)
                    
    #                 # Extract the relevant data from the WrenchStamped message
    ...
    def read_rosbag_to_csv(self, bag_file, topic_name, output_csv):
        ...
        self.get_logger().info(f"Data has been written to {output_csv}")
